...
void level3::Pause_Init() {
    ...
    MyPushButton *initGame = new MyPushButton(":/photo/initGame.jpg");
...
void level3::paintEvent(QPaintEvent *) {
    ...
    for (QVector < QVector < int >> ::iterator it = spikeweed->m.begin()->begin(); it != spikeweed->m.begin()->end();
         it++)
    {
        if (*(it->begin()) - andy->x > -50 && *(it->begin()) - andy->x < 800) {
            painter.drawPixmap(*(it->begin()) - andy->x, *(it->begin() + 1)+20, 50, 40, QPixmap(":/photo/spikeweed.png"));
        }
    }
...
void level3::Game_Init() {
    ...
    game_start = false;
    open =false;
    spikeweed->Spikeweed_State(andy);
